...
public class Shooter extends SubsystemBase {
    ...
    public void periodic() {
        ...
        if (autoAimEnabled && robotPoseSupplier != null && targetSupplier != null && robotVelocitySupplier != null) {
            ...
            Translation2d turretPoseField = getTurretPoseField(robotPose); 
            ...
            // Velocidade tangencial (girando no próprio eixo)
            double omega = robotSpeeds.omegaRadiansPerSecond;
            Translation2d tangentialVelocity = new Translation2d(kXoffset, kYoffset)
                    .rotateBy(new Rotation2d(Math.PI / 2.0)) // 90 graus para ser tangencial
                    .times(omega)
                    .rotateBy(robotPose.getRotation());

            Translation2d totalVelocityCompensated = fieldVelocity.plus(tangentialVelocity);
            
            double staticDist = turretPoseField.getDistance(realTargetLocation);
            ...
            double dynamicDist = turretPoseField.getDistance(virtualTargetLocation);
            ...
            Logger.recordOutput("Shooter/VirtualTarget", new Pose2d(virtualTargetLocation, new Rotation2d()));
            Logger.recordOutput("Shooter/RealTarget", new Pose2d(realTargetLocation, new Rotation2d()));
            Logger.recordOutput("Shooter/TotalCompensatedVel", totalVelocityCompensated);

            Logger.recordOutput("Shooter/VirtualTarget3D", new Pose3d(
                virtualTargetLocation.getX(), 
                virtualTargetLocation.getY(), 
                currentTargetHeight, 
                new Rotation3d() // Sem rotação
            ));

            // Registrar a Pose3d do Alvo Real para comparação visual
            Logger.recordOutput("Shooter/RealTarget3D", new Pose3d(
                realTargetLocation.getX(), 
                realTargetLocation.getY(), 
                currentTargetHeight, 
                new Rotation3d()
            ));
    ...
    public Translation2d getTurretPoseField(Pose2d robotPose) {
    Translation2d turretOffsetLocal = new Translation2d(kXoffset, kYoffset);
    return robotPose.getTranslation().plus(turretOffsetLocal.rotateBy(robotPose.getRotation()));
    }
